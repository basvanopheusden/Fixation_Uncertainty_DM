function [ output_args ] = ibs_repeated_LPwrapper(Para,Func,ScalingFactor, FixNumLNR, LRating, RRating, Choice,ReactionTime,RTbin,LLcutoff,Nrep,savefile)
...
if isempty(Para)
    ...
    output_args = sum(mean(allLs,1));
            
    funtable = [funtable;Para,-output_args,mean(std(allLs,1))];
    Tcalc = toc;
