function modfit_bads_repibs(whichSubj,nrun,ndT,FMod)
% evaluate only once.
close all
rng('shuffle')
savefile = ['Realfit/' FMod '_repibs_BADS_subj_',num2str(whichSubj),'_ndT',num2str(ndT),'_run',num2str(nrun)];
...
options.NoiseSize           = 3;    % Estimate of noise std
...
Nrep=5;
% start fitting
clear ibs_repeated_LPwrapper
[thisFittedPara,LogProb100ms,exitflag,output] = bads('ibs_repeated_LPwrapper',iniPar,lb,ub,problb,probub,[],options,modfun,ScalingFactor,SubFixNumLNR, SubLRating,SubRRating, SubChoice,SubRT,allRTbins,LLcutoff,Nrep,savefile);

% get prob fluct
nflu = 10;
ProbFluct = nan(1,nflu);
for kflu = 1:nflu
    ProbFluct(kflu) = ibs_repeated_LPwrapper(thisFittedPara,modfun,ScalingFactor,SubFixNumLNR, SubLRating,SubRRating, SubChoice,SubRT,allRTbins,LLcutoff,Nrep,[]);
end
